            ...
            fetch(`/get_images/${folderName}`)
                ...
                .then(imageUrls => {
                    if (imageUrls.error) {
                         ...
                         nextBtn.style.display = 'none';
                         modal.style.display = 'block'; // Still show modal to indicate error?
                    } else if (imageUrls.length > 0) {
                        currentImages = imageUrls;
                        showImage(0); // Show the first image
                        modal.style.display = 'flex'; // Display the modal as flex
                        ...
                        nextBtn.style.display = 'none';
                        modal.style.display = 'block'; // Show modal to indicate no images
                    }
                })
                .catch(error => {
                    ...
                    nextBtn.style.display = 'none';
                    modal.style.display = 'block'; // Show modal to indicate error
                });
        });
    });
